...
model_search_key = {'mrp.bom':'code',
                    'product.category':'name'}


header_bom_line_d = {'ready_to_produce': ("all_available","mrp.bom"),
                    'consumption':('flexible','mrp.bom') }


header_bom_line_g = {
                    'code':('Code_bom',"mrp.bom",None,None),
                    'product_tmpl_id':('ID_Produs',"mrp.bom","product.template",[('default_code',None)]),
                    'product_qty_1': ('Cantitate_Produsa',"mrp.bom",None,None),
                    'product_id': ('ID_Componenta',"mrp.bom.line",'product.product',[('default_code',None)]),
...
header_stock_valuation_layer_g={
                            'company_id': ('Compania','stock.valuation.layer','res.company',[('name',None)]),
                            'product_id':("Product_ID",'stock.valuation.layer','product.product',[('name',None)]),
                            'quantity':("Cantitatea",'stock.valuation.layer',None,None),
                            'uom_id':("Unitate_de masura",'stock.valuation.layer','product_id.uom_id',[('name',None)]),
                            'unit_cost':('Pret','stock.valuation.layer',None,None ),
                            'value':("Value",'stock.valuation.layer',None,None),
                            'currency_id':("Moneda",'stock.valuation.layer','res.currency',[('name',None)]),
                            'description':("Descriere",'stock.valuation.layer',None,None)
                             }

header_inventory = {'product_id': 'ID_Produs',
                    'location_id':'Locatie',
                    'quantity':'Cantitatea',
...
header_quant_g = {'product_id': ('Product_ID',"stock.quant",'product.product',[('name',None)]),
...
header_inventory_g ={'inventory_id':('Nume_inventoryy','stock.inventory.line','stock.inventory',[('name',"Inventar_02initial")]),
                    'product_id':('Product_ID','stock.inventory.line','product.product',[('name',None)]),
                     'product_qty':('Cantitatea','stock.inventory.line',None,None),
                    'location_id':('Depozit','stock.inventory.line','stock.location',[('name',None)]),
                     }



header_asset = {'method': 'Metoda_Amortizare',
...
header_asset_g = {'method': 'Metoda_Amortizare',
                'method_number': "Numarul_de_Amortizari",
                'method_period': 'Tipul_perioada',
                'inventory_number': 'Numarul_inventar',
                'method_number_min': 'Numarul_Min_amortizari',
...
header_product_cat = {'name': ('nume','product.category',None,None,),
                      ...
                      'property_account_income_categ_id':('cont_ven','product.category','account.account',[('code',None)]),
                      'property_account_expense_categ_id':('cont_che','product.category','account.account',[('code',None)]),
                      'property_cost_method':('cost_method','product.category',None,None,),
                      'property_stock_valuation_account_id':('cont_valuation','product.category','account.account',[('code',None)]),
                      #'removal_strategy_id': ('Scoatere','product.category','product.removal',[('name',None)]),
                       'property_valuation':('Metoda','product.category',None,None,),
                       "property_stock_journal":('Journal','product.category','account.journal',[('name',None)]),
                       "property_stock_account_input_categ_id": ('cont_val_input','product.category','account.account',[('code', None)]),
                        "property_stock_account_output_categ_id": ('cont_val_output','product.category','account.account',[('code', None)])
}

header_product = {'id': 'id',
                'name': 'Nume',
                'sequence': 0,
                'description': 0,
...
header_product_g = {
                ...
                'sequence': 0,
                'description': ("Descriere",'product.template',None,None),
                'description_purchase': 0,
                'description_sale': 0,
                ...
                'product_variant_id': 0,
                'product_variant_count': 0,
                'barcode': ("Cod_bare",'product.template',None,None),
                'default_code':("SKU",'product.template',None,None),
                'code_uk':("code_uk",'product.template',None,None),
                'pricelist_item_count': 0,
                'can_image_1024_be_zoomed': 0,
                'has_configurable_attributes': 0,
                'taxes_id': ('Taxe','product.template',"account.tax",[('name',None)]),
                'supplier_taxes_id': ('Taxe_achizitie','product.template',"account.tax",[('name',None)]),
                'property_account_income_id': 0,
                'property_account_expense_id': 0,
                'service_type': 0,
                'sale_line_warn': 0,
...
header_partner_g = {
                      'name': ('Nume','res.partner',None,None),
                      'parent_id':0,
                      'vat':('CUI','res.partner',None,None),
                      'nrc':('NRC','res.partner',None,None),
                      'type': 0,
                      ...
                      'state_id': ('Judet','res.partner','res.country.state',[('code',None),('country_id','country_id')]),
                      'country_id': ('Tara','res.partner','res.country',[('code',None)]),
...
header_stock_valuation_layer_d = { 'company_id' : (1,'stock.valuation.layer')}



header_account = {'code': 'Cont',
...
header_account_g = {"account_id": ("Cont",'account.move.line','account.account',[('code',None)]),
...
head_move_line={'id': 10, 'move_id': [6, 'INV/2021/06/0001'], 'move_name': 'INV/2021/06/0001', 'date': '2021-06-16', 'ref': False, 'parent_state': 'posted', 'journal_id': [1, 'Customer Invoices'], 'company_id': [1, 'My Company1'], 'company_currency_id': [28, 'RON'], 'tax_fiscal_country_id': False, 'account_id': [203, '411100 Clienți'], 'account_internal_type': 'receivable', 'account_internal_group': 'asset', 'account_root_id': [52049, '41'], 'sequence': 10, 'name': 'INV/2021/06/0001', 'quantity': 1.0, 'price_unit': -35.7, 'discount': 0.0, 'debit': 35.7, 'credit': 0.0, 'balance': 35.7, 'cumulated_balance': 0.0, 'amount_currency': 35.7, 'price_subtotal': -35.7, 'price_total': -35.7, 'reconciled': False, 'blocked': False, 'date_maturity': '2021-06-16', 'currency_id': [28, 'RON'], 'partner_id': [9, 'partner11'], 'product_uom_id': False, 'product_id': False, 'product_uom_category_id': False, 'reconcile_model_id': False, 'payment_id': False, 'statement_line_id': False, 'statement_id': False, 'tax_ids': [], 'tax_line_id': False, 'tax_group_id': False, 'tax_base_amount': 0.0, 'tax_exigible': True, 'tax_repartition_line_id': False, 'tax_tag_ids': [], 'tax_audit': '', 'amount_residual': 35.7, 'amount_residual_currency': 35.7, 'full_reconcile_id': False, 'matched_debit_ids': [], 'matched_credit_ids': [], 'matching_number': False, 'analytic_line_ids': [], 'analytic_account_id': False, 'analytic_tag_ids': [], 'recompute_tax_line': False, 'display_type': False, 'is_rounding_line': False, 'exclude_from_invoice_tab': True, 'move_attachment_ids': [], 'is_anglo_saxon_line': False, 'predict_from_name': False, 'expected_pay_date': False, 'internal_note': False, 'next_action_date': False, 'sale_line_ids': [], 'asset_ids': [], 'followup_line_id': False, 'followup_date': False}

header_move_line = { 'account_id':'Cont',
...
header_asset_g = {
    ...
    "account_asset_id": ("Cont_mijloc_fix",'account.asset','account.account',[('code',None)]),
    "account_depreciation_id": ("Cont_amortizari",'account.asset','account.account',[('code',None)]),
    "account_depreciation_expense_id": ("Cont_cheltuiala",'account.asset','account.account',[('code',None)]),
    ...
    "already_depreciated_amount_import": ("Valoare_amortizare",'account.asset',None,None),
    "depreciation_number_import": ("Numar_amortizari_import",'account.asset',None,None),
    "first_depreciation_date_import": ("Data_amortizare_import",'account.asset',None,None),
