...
from .dict_fields import header_bom_line_g,header_inventory_g,header_asset,header_product_cat,header_product_g,\
    header_partner_d,header_partner,header_partner_g,header_account_g,header_move_line,header_move_account,\
    header_move_line_g,header_asset_g,header_bom_line_d,header_stock_valuation_layer_g,header_stock_valuation_layer_d
...
class ImportData(models.Model):
    ...
    file_import = fields.Selection(
        ...
        selection=[("partner", "Partners"), ("cat_product", "Category Product"), ("product", "Product"),
                   ("account", "Initial Balance"),("balance_partner", "Balance Partner"), ('bom','Bom'),("asset", "Asset"),('inventory','Inventory'),
                   ('valuation',"Stock_valuation")],
        default="partner",
    )

    update_data = fields.Boolean(string='Update', default = True)
    details = fields.Html('Import details', help='Contains details and errors in the import process ')
    ...
    def import_model(self):
        ...
        if not self.check:
            if self.file_import=='partner':
               self.import_generic_models('partner.csv',header_partner_g,None,header_partner_d)
            #self.update_generic_models('partner.csv', 'name', header_partner_g)
                ...
                self.import_generic_models('cat_exp.csv',header_product_cat)
            elif  self.file_import == "account":
                self.import_generic_models(file_name='account_dec.csv', header_generic=header_account_g,
                                           level_multiline=1, level_creation_allowed=1)
                self.import_generic_models(file_name='account_feb.csv',header_generic=header_account_g,level_multiline=1,level_creation_allowed=1)
                self.import_generic_models(file_name='account_ian.csv', header_generic=header_account_g,
                                           level_multiline=1, level_creation_allowed=1)
                self.import_generic_models(file_name='account_mar.csv', header_generic=header_account_g,
                                           level_multiline=1, level_creation_allowed=1)
                self.import_generic_models(file_name='account_apr.csv', header_generic=header_account_g,
                                           level_multiline=1, level_creation_allowed=1)
                self.import_generic_models(file_name='account_mai.csv', header_generic=header_account_g,
                                           level_multiline=1, level_creation_allowed=1)
                self.import_generic_models(file_name='account_iun.csv', header_generic=header_account_g,
                                           level_multiline=1, level_creation_allowed=1)
            elif self.file_import == "product":
                self.import_generic_models('product2.csv',header_product_g)
                ...
                #self.import_generic_models(file_name='inventory4.csv',header_generic=header_inventory_g,level_creation_allowed=1)
                ...
                self.import_generic_models('account_move_partner.csv',header_move_line_g,level_multiline=1,level_creation_allowed=1)
                ...
                #self.update_valuation_models('valuation_obj.csv')
    ...
    def export_model (self):
        if self.file_import == 'partner':
           self.export_generic_models('partner_exp.csv',header_partner_g)
        elif self.file_import == 'cat_product':
    ...
    def get_field_merge_export(self,model_level,model,level):
        ...
        model_parent = model_level[level-1][0]
        _logger.warning('Model relation')
        _logger.warning(model_parent)
        field_merge = ir_model_obj.search([('model', '=', model),('relation', '=', model_parent)],limit=1)
    ...
    def get_field_merge(self,model_level,model,item_dict):
        ...
        if model != model_level[1][0]:
            ir_model_field = ir_model_obj.search([('model', '=', model),('relation', '=', model_level[1][0])],limit=1)
            ...
            for model_rel in item_dict.keys():
                if model_rel in model_level[0]:
                    ir_model_field = ir_model_obj.search([('model', '=', model), ('relation', '=', model_rel)],
                                                         limit=1)
    ...
    def order_models(self,header_generic):
        ...
        for model in models[1:]:
            ...
            _logger.warning(ir_model_field)
    ...
    def convert_type_field(self,dict_filter,model):
        for field,value in dict_filter.items():
            field_db = self.env['ir.model.fields'].search([('model', '=', model), ('name', '=', field)])
    ...
    def import_generic_models(self,file_name,header_generic,level_multiline = None, default_model=None,level_creation_allowed =2):
        ...
        with open(self.get_path(file_name), "r") as infile:
            ...
            for row in reader:
                ...
                if len(headers_translate)!= len(row):
                    _logger.warning(f"The number of headers fields  does not match the number of fields in the row number {row}")
                if 1==1:
                    ...
                    for level,models  in enumerate(model_level):
                        ...
                        for model in models :
                           ...
                           # _logger.warning(keys_models)
                            ...
                            dict_filter_00 = dict(
                                (k, dict_read[k]) for k,v in header_generic.items() if v[1] == model and k in dict_read.keys() )
                            _logger.warning(dict_filter_00)
                            # se transforma campul cu _1 ( care apare in mai multe modele) cu denumirea campului din model
                            dict_filter_0 = dict((self.translate_double_key(k), value) for k, value in dict_filter_00.items())

                            _logger.warning((dict_filter_0))
                            # se verifica daca campurile sunt in model ( luat din odoo) si sa nu fie nule
                            dict_filter_1 = dict((k, dict_filter_0[k]) for k in keys_model if k in dict_filter_0 and dict_filter_0[k])
                            ...
                            for keyss, values in header_generic.items():
                                ...
                                if keyss in dict_filter_1.keys():
                                    ...
                                    # Merge link id model
                                        ir_model_obj = self.env['ir.model.fields']
                                        ir_model_field = ir_model_obj.search([('model', '=', m), ('name', '=', keyss)],limit =1)
                            ...
                            if default_model :
                                for dkeys in default_model.keys():
                                    ...
                                    ir_model_field = ir_model_obj.search([('model', '=', 'res.company'), ('name', '=', dkeys)],
                                                                         limit=1)
                                    if default_model[dkeys][1] == model:
                                        if dkeys in dict_filter_1.keys():
                                            if not dict_filter_1[dkeys]:
                                                dict_filter_1[dkeys]= default_model[dkeys][0]
                                        else:
                                            if ir_model_field.ttype != 'many2one':
                                                dict_filter_1[dkeys] = default_model[dkeys][0]
                                            else :
                                                company = self.env['res.company'].search([('id', '=', default_model[dkeys][0])], limit=1)
                            ...
                            if 1 == 1 :
                                if search_key :
                                    ...
                                    for field in ir_model_field:
                                        ...
                                        if field.name in dict_filter_1.keys():
                                            if  field.ttype in ["char","text","integer","float","boolean","monetary","selection"]:
                                ...
                                if item_search and level < level_creation_allowed:
                                        ...
                                        _logger.warning(f' FFFFFind {model}:{item_search} don`t create new item')
                                else :
                                        ...
                                        _logger.warning(f'Was create obj{model}:{item_search}')
    ...
    def update_generic_models(self, file_name,field_search,header_generic,extra_string=None):
        ...
        with open(self.get_path(file_name), "r") as infile:
            ...
            for row in reader:
                ...
                for keyss, values in header_generic.items():
                    ...
                    t, m, cm, f = values
                    if keyss in keys_models[0] and keyss == 'country_id':
                        if cm:
                            _logger.warning(cm)
                            rel_keys = []
                            _logger.warning(f)
                            ...
                            dict_filter_1[keyss] = item_object.id
                _logger.warning(dict_filter_1)
                field_update.write({'country_id':dict_filter_1['country_id']})
    ...
    def update_valuation_models(self, file_name,field_search='description',header_generic=header_stock_valuation_layer_g,):
        ...
        with open(self.get_path(file_name), "r") as infile:
            ...
            for row in reader:
                dict_read = {key: (value) for key, value in zip(headers_translate, row[0:])}
                product_name = self.env['product.template'].search([('name', '=', dict_read["product_id"])], limit =1)
    ...
    def check_id_link(self, file_name, header_generic, level_multiline=None, default_model=None):
                ...
                if 1 == 1:
                    ...
                    for level, models in enumerate(model_level):
                        ...
                        for model in models:
                            ...
                            # se verifica daca campurile sunt in model ( luat din odoo) si sa nu fie nule
                            dict_filter_1 = dict(
                                (k, dict_filter_0[k]) for k in keys_model if k in dict_filter_0 and dict_filter_0[k])
                            # _logger.warning("Dict Filter")
                            # _logger.warning(dict_filter_1)
                            search_key = None
                            for keyss, values in header_generic.items():
                                ...
                                if keyss in dict_filter_1.keys():
                                    if cm:
                                        # Merge link id model
                                        ir_model_obj = self.env['ir.model.fields']
                                        ir_model_field = ir_model_obj.search([('model', '=', m), ('name', '=', keyss)],
                                                                             limit=1)
                                        _logger.warning(f'{keyss}:{ir_model_field.ttype}')
                                        if ir_model_field.ttype == 'many2one':
                                            if f:
                                                ...
                                                item_object,mess = self.merge_models(cm, rel_keys,check=True)
                                                if mess :
    ...
    def export_generic_models(self, file_name, header_generic):
        ...
        with open(self.get_path(file_name), mode='w') as csv_file:
            ...
            model_obj = self.env[model_write]
            ...
            for val in values_write:
                    ...
                    if 1==1:
                        if field.ttype not in ['many2one', 'many2many', 'one2many']:
                            if field.name in dir(val):
                                ...
                                _logger.warning(field.ttype)
                                if val[field.name] == False and field.ttype != 'boolean':
                                    new_dict[field.name]= ''
                                else:
                            ...
                            if field.name in header_generic.keys():
                                if header_generic[field.name] != 0:
                                    ...
                                    _logger.warning(val[field.name])
                                    val_rel_found = self.env[cm].search([('id', '=', val[field.name].id)])
                                    if field.ttype == 'many2one':
                                        ...
                                        if val_rel_found[name_field_rel] == False :
                                            ...
                                            new_dict[field.name] = val_rel_found[name_field_rel]
                                    else:
                                        _logger.warning(field.name)
                                        _logger.warning(val_rel_found[name_field_rel])
                                        new_dict[field.name] = val_rel_found[name_field_rel]
                ...
                for keyy in header_generic.keys():
                    ...
                    if header_generic[keyy] != 0 and header_generic[keyy][1]==model_write:
                        nnew_dict[header_generic[keyy][0]] = new_dict[key_model]
                    if level_write == 2 and [keyy] != 0 and header_generic[keyy][1]==model_level[1][0]:
                    ...
                    if level_write == 1 and [keyy] != 0 and header_generic[keyy][1] == model_level[0][0]:
                        parent_model_write_field = ir_model_obj.search(
                            [('model', '=', model_level[0][0]), ('name', '=', key_model)])
                        _logger.warning(parent_model_write_field.name)
                        _logger.warning(parent_model_write_field.ttype)
                        if parent_model_write_field.ttype in ['many2one']:
                            t, m, cm, f = header_generic[keyy]
    ...
    def update_product_evinox_models(self, file_name, header_generic):
        ...
        with open(self.get_path(file_name), "r") as infile:
            ...
            for row in reader:
                ...
                _logger.warning(value_search)
                row_update = self.env['product.template'].search([('default_code', '=', value_search)],limit =1)
                if len(row_update) == 1:

                        _logger.warning(f'Find code {row_update.name}')
                        res = {'default_code': dict_read['default_code'],
                        ...
                        # Merge link id model
                        ir_model_obj = self.env['ir.model.fields']
                        ir_model_field = ir_model_obj.search([('model', '=', 'product.category'), ('name', '=', keyss)],
                                                             limit=1)
                        _logger.warning(f'{keyss}:{ir_model_field.ttype}')
                        f = 'name', None
                        rel_keys = []
                        ...
                        if row_update.name == dict_read['name']:
                            ...
                            _logger.warning(f'Name issue {dict_read["name"]} is not {row_update.name}')
                            row_update.write(res)
                    ...
                    row_update = self.env['product.template'].search([('name', 'ilike', value_search)],limit=1)
                    _logger.warning(row_update)
                    if row_update:
    ...
    def update_product_correct(self):
        ...
        products_consumabile = self.env['product.template'].search([('categ_id', '=', cat_id.id)])
